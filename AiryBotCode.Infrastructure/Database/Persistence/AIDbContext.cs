...
namespace AiryBotCode.Infrastructure.Database.Persistence
{
    public class AIDbContext : DbContext
    {
        ...
        public DbSet<SuggestionShareCode> SuggestionShareCodes { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.ApplyConfiguration(new CountingStateConfiguration());
            modelBuilder.ApplyConfiguration(new SuggestionConfiguration());
            modelBuilder.ApplyConfiguration(new SuggestionShareCodeConfiguration());
        }
        ...
        public static void EnsureNewerTables(AIDbContext db)
        {
            const string sql = @"
                ...
                -- Counting game state, one row per bot+channel.
                CREATE TABLE IF NOT EXISTS ""CountingStates"" (
                    ""Id""            integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    ""BotId""         numeric(20,0) NOT NULL,
                    ...
                    ""BossAnswer""    double precision NULL,
                    ""BossSpawnedAt"" timestamp with time zone NULL,
                    ""UpdatedAt""     timestamp with time zone NOT NULL DEFAULT now()
                );
                CREATE UNIQUE INDEX IF NOT EXISTS ""IX_CountingStates_BotId_ChannelId""
                    ON ""CountingStates"" (""BotId"", ""ChannelId"");

                -- Feature suggestions / ideas board (project-wide, not per-bot).
                CREATE TABLE IF NOT EXISTS ""Suggestions"" (
                    ...
                    ""ResponseWhy""      text NULL,
                    ""ResponseEstimate"" text NULL,
                    ""RespondedAt""      timestamp with time zone NULL,
                    ""Approved""         boolean NOT NULL DEFAULT false,
                    ""ApprovedAt""       timestamp with time zone NULL
                );

                -- Single-row capability code for the PUBLIC shareable suggestions board.
                CREATE TABLE IF NOT EXISTS ""SuggestionShareCodes"" (
                    ""Id""        integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    ""Code""      text NOT NULL,
                    ""CreatedAt"" timestamp with time zone NOT NULL DEFAULT now()
                );";
            try
            {
                ...
                Console.WriteLine("- Ensured UserConsents + LevelUsers + CountingStates tables + BotSettings theme columns.");
            }
            catch (Exception ex)
            {
